...
#define PREPARE_GDT_ENTRY(base, limit, flags, access)                   \
    ((((((base)) >> 24) & 0xFF) << 56) | ((((flags)) & 0x0F) << 52) |   \
     (((((limit)) >> 16) & 0x0F) << 48) | ((((access)) & 0xFF) << 40) | \
     ((((base)) & 0xFFFFFF) << 16) | (((limit)) & 0xFFFF))

/* Null descriptor, never be referenced */
#define GDT_NULL_ENTRY PREPARE_GDT_ENTRY(0, 0, 0, 0)

#define GDT_KERNEL_CODE                                                 \
    PREPARE_GDT_ENTRY(0, 0xfffff, GDT_FLAG_LONG | GDT_FLAG_GRANUALRITY, \
...
#define GDT_KERNEL_CODE32                                             \
    PREPARE_GDT_ENTRY(0, 0xfffff, GDT_FLAG_DB | GDT_FLAG_GRANUALRITY, \
                      GDT_ACCESS_PRESENT | GDT_ACCESS_PRIV_RING0 |    \
                          GDT_ACCESS_TYPE | GDT_ACCESS_EXECUTABLE |   \
                          GDT_ACCESS_READABLE_WRITABLE)

#define GDT_KERNEL_DATA32                                             \
    PREPARE_GDT_ENTRY(0, 0xfffff, GDT_FLAG_DB | GDT_FLAG_GRANUALRITY, \
                      GDT_ACCESS_PRESENT | GDT_ACCESS_PRIV_RING0 |    \
                          GDT_ACCESS_TYPE | GDT_ACCESS_READABLE_WRITABLE)
...
inline void set_GDT_entry8(struct GDTDescr_8* entry, uint32_t baseaddress,
...
inline void set_GDT_entry16(struct GDTDescr_16* entry, uint64_t baseaddress,
                            uint32_t limit, uint8_t flags, uint8_t access) {
    entry->lowerlimit = limit & 0xffff;
    entry->upperlimit_flags = (flags << 4) | (limit >> 16) & 0x0f;
    entry->access = access;
